...
HYGIENE_COMMIT = 'c33af159d00c07f2d66b2f93174cfcdb4cde8d37'
HYGIENE_LAB_MERGE = '25ec05381207252abb8abec8f6f99e30765f9704'
PACKET = PARENT / 'packets' / 'R2-P1_FIXTURE_JOIN_000_FREEZE_2026-09-22.md'
PACKET_SHA256 = 'e9bac91ca908b2ba704d966f0cf48cb181070ac11de1d117b93512bd2719ae1c'
PACKET_RESULTS = PARENT / 'packets' / 'R2-P1_FIXTURE_JOIN_000' / 'results.json'
PACKET_FROZEN = PARENT / 'packets' / 'R2-P1_FIXTURE_JOIN_000' / 'FROZEN_EXPERIMENT.json'
FROZEN_EXPERIMENT = ROOT / 'FROZEN_EXPERIMENT.json'
EMPTY_RESULTS = ROOT / 'results' / 'EMPTY_RESULTS.json'
FACTORIAL_RESULTS = PARENT / 'nfl_factorial_lab_20260921' / 'results'
...
CARRIED_UNREAD = ('inventory_after', 'cash_after', 'paired', 'fee', 'queue_remaining')
...
def execution_adapter():
    ...
    raise hygiene.LiveOrdersForbidden()


def instrument_binding():
    """Pins for the imported labs. Rates stay inside those modules."""
    ...
    return {
        ...
        'hygiene_lab_merge': HYGIENE_LAB_MERGE,
        'examiner_formula_id': hygiene_binding['examiner_formula_id'],
        'fee_credit_rule_id': hygiene_binding['fee_credit_rule_id'],
        'inherited_model_id': hygiene_binding['inherited_model_id'],
        'live_orders': False,
        'signal_retune': False,
        'forbid_capital_A2_A3': True,
        'deferred_pick_B': DEFERRED_PICK_B,
...
def ledger_identity(path):
    ...
    if scenario not in hygiene.ASSUMED_SCENARIOS:
        raise FixtureJoinError('assumed scenario')
    return {
        'assumed_scenario': scenario,
...
def _rate(role, table):
    return feebook.as_decimal(table['rates'][role], role)


def _freshness(cursor, fill):
    keepalive = fill.get('keepalive', False)
    if not isinstance(keepalive, bool):
        raise FixtureJoinError('keepalive')
    book = fill.get('book', None)
    if book is None and not keepalive:
        return None
    current = None
    if book is not None:
        if not isinstance(book, dict):
            raise FixtureJoinError('book')
        transaction_time = fill.get('transaction_time')
        if transaction_time is None and not keepalive:
            raise FixtureJoinError('transaction_time')
        current = rails.BookObservation(rails.canonical_book_content(book), transaction_time)
    return cursor.observe(
        current,
        ledger_decimal(fill['at'], 'at'),
        keepalive=keepalive,
    )


def label_joined_fill(fill, order, assumed_scenario, cursor, table, fee_state):
    """One fill through the hygiene helpers. The row carries no profit."""
    role = fill['kind']
    if role not in ('maker', 'taker'):
        raise FixtureJoinError('kind')
    price = ledger_decimal(fill['price'], 'price')
    contracts = ledger_decimal(fill['size'], 'size')
    if role == 'maker':
        submitted = ledger_decimal(order['submitted_quantity'], 'submitted_quantity')
        round_up = contracts == submitted
        order_id = order['order_id']
    else:
        round_up = True
        order_id = fill['order_id']
    delta = hygiene.fee_delta(
        role,
        contracts,
        price,
        _rate(role, table),
    ...
    credit = None
    if role == 'maker':
        credit = hygiene.maker_credit_floor_zero_refuse(price, contracts, table=table)
    fresh = _freshness(cursor, fill)
    queue_bin = None
    mismatch = None
    measured = None
    if role == 'maker' and order is not None and order.get('initial_queue') is not None:
        measured = ledger_decimal(order['initial_queue'], 'initial_queue')
        judged = hygiene.queue_bin_mismatch(measured, assumed_scenario)
        queue_bin = judged['queue_attribution_bin']
        mismatch = judged['queue_bin_mismatch']
    return {
        'order_id': order_id,
        'role': role,
        ...
        'freshness_gap_sec': None if fresh is None else fresh['freshness_gap_sec'],
        'queue_attribution_bin': queue_bin,
        'queue_bin_mismatch': mismatch,
        'measured_queue_ahead': measured,
        'pnl': None,
    }


def join_rows(fills, orders, *, assumed_scenario, source, fills_path, orders_path):
    """Join already loaded rows. Does not write a scorecard."""
    if assumed_scenario not in hygiene.ASSUMED_SCENARIOS:
        raise FixtureJoinError('assumed scenario')
    ...
    for fill in fills:
        ...
        if role == 'maker':
            ...
            fee_state = maker_states.setdefault(order_id, hygiene.InheritedFeeState())
            maker_rows += 1
        elif role == 'taker':
            fee_state = hygiene.InheritedFeeState()
            taker_rows += 1
        else:
            raise FixtureJoinError('kind')
        labels.append(label_joined_fill(
            fill, order, assumed_scenario, cursor, table, fee_state,
        ))
