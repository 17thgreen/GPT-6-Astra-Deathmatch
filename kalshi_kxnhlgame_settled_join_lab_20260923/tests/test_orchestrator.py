...
class PinTests(unittest.TestCase):
    def test_binding_pins_the_seed_and_leaves_the_scorecard_null(self):
        ...
        self.assertFalse(orchestrator.NHL_SCOUT_CITE.exists())
        self.assertFalse(orchestrator.NHL_PANEL_CITE.exists())
        ...
        self.assertEqual(binding['overnight_finalized_nonempty_N'], 17)
        ...
        self.assertEqual(binding['parent_seed_statuses'], dict(orchestrator.PARENT_SEED_STATUSES))
        self.assertEqual(binding['parent_panel_result'], '')
        self.assertEqual(binding['reget_active_empty_result'], '')
        self.assertEqual(binding['reget_active_empty_tickers'], list(orchestrator.REGET_ACTIVE_EMPTY_TICKERS))
        self.assertEqual(binding['series_list_settled_finalized_open_429'], '429_honest')
        self.assertIs(binding['list_429_backfilled'], False)
        self.assertIs(binding['missing_side_invented'], False)
        self.assertEqual(binding['one_sided_event_tickers'], list(orchestrator.ONE_SIDED_EVENTS))
        self.assertEqual(
            binding['panel_markets_absent_from_reget'],
            list(orchestrator.PANEL_ABSENT_FROM_REGET),
        )
        ...
        self.assertNotEqual(binding['settled_join_n'], binding['overnight_finalized_nonempty_N'])
        ...
        self.assertIs(pins['governance_nhl_cites_present'], False)
    ...
    def test_reget_honesty_keeps_the_list_429_gaps(self):
        ...
        self.assertEqual(seed['overnight_finalized_nonempty_N'], 17)
        ...
        self.assertEqual(len(reget['active_empty']), 4)
        self.assertEqual(reget['honest_gaps'], {
            orchestrator.GAP_MARKETS: '429_honest',
            orchestrator.GAP_EVENTS: '429_honest',
        })
        self.assertEqual(orchestrator.list_429_gaps(reget), orchestrator.GAP_KEYS)
        self.assertNotIn('artifacts', reget)
        self.assertNotIn('http_log', reget)
        for key in orchestrator.GAP_KEYS:
            self.assertNotIn(key, reget['markets'])
        for ticker, row in reget['active_empty'].items():
            self.assertEqual(row['status'], 'active')
            self.assertEqual(row['result'], '')
            self.assertNotIn(ticker, reget['markets'])
        for ticker in orchestrator.PANEL_ABSENT_FROM_REGET:
            self.assertNotIn(ticker, reget['markets'])
            self.assertNotIn(ticker, reget['active_empty'])
        for ticker, result in orchestrator.OVERNIGHT_RESULTS.items():
            ...
            self.assertNotIn(ticker, {row['ticker'] for row in panel['markets']})
        for market in panel['markets']:
            self.assertEqual(market['result'], '')
            self.assertEqual(market['status'], 'active')
        agreed = orchestrator.occurrence_agreement_tickers(panel)
        self.assertEqual(len(agreed), 17)
        self.assertEqual(agreed, orchestrator.OVERNIGHT_TICKERS)
        ...
        self.assertTrue(all(row['on_settled_list'] is False for row in passes))
        self.assertTrue(all(row['single_market_get'] is True for row in passes))
        self.assertEqual({row['key'] for row in empties if row['parent_seed']}, set(orchestrator.PARENT_SEED_TICKERS))
        self.assertEqual([row['result'] for row in empties], ['', '', '', ''])
        ...
        buf = reget['markets']['KXNHLGAME-26SEP22CBJBUF-BUF']
        self.assertEqual(buf['result'], 'yes')
        self.assertEqual(buf['occurrence_datetime'], '2026-09-23T02:00:00Z')
        event_counts = {}
        for market in reget['markets'].values():
            event_counts[market['event_ticker']] = event_counts.get(market['event_ticker'], 0) + 1
        for event in orchestrator.ONE_SIDED_EVENTS:
    ...
    def test_source_pins_and_accept(self):
        ...
        self.assertEqual(pins['governance_nhl_cites'], 'absent')
        ...
        self.assertEqual(pins['overnight_finalized_nonempty_N'], 17)
        ...
        self.assertEqual(pins['parent_panel_result'], '')
        self.assertIs(pins['list_429_backfilled'], False)
        self.assertEqual(pins['series_list_settled_finalized_open_429'], '429_honest')
        self.assertEqual(pins['honest_gaps'], list(orchestrator.HONEST_GAPS))
        self.assertIs(pins['missing_side_invented'], False)
...
class SchemaTests(unittest.TestCase):
    def test_j0_and_j1_schema_leave_the_scorecard_null(self):
        ...
        for ticker in orchestrator.OVERNIGHT_TICKERS:
        ...
        self.assertIs(j0['missing_side_invented'], False)
...
class RefuseTests(unittest.TestCase):
    ...
    def test_invented_result_depth_fills_and_list_429_backfill_are_refused(self):
        panel = orchestrator.load_panel()
        reget = orchestrator.load_reget()
        market = reget['markets']['KXNHLGAME-26SEP22CBJBUF-BUF']
        panel_market = panel['markets'][0]
        with self.assertRaises(orchestrator.InventedDepthRefused):
            orchestrator.quote_depth(market)
        with self.assertRaises(orchestrator.InventedDepthRefused):
            orchestrator.quote_depth(panel_market)
        self.assertNotIn('orderbook_fp', market)
        with self.assertRaises(orchestrator.InventedFillRefused):
            orchestrator.invent_fill(panel_market, panel_market.get('volume_fp'))
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.invent_result('KXNHLGAME-26SEP22CBJBUF-BUF', 'no')
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.invent_parent_settle(orchestrator.PARENT_SEED_TICKERS[0], 'yes')
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.backfill_list_429(
                orchestrator.GAP_MARKETS,
                result='yes',
                occurrence_datetime='2026-09-23T02:00:00Z',
            )
        with self.assertRaises(orchestrator.InventedSoTRefused):
            orchestrator.assign_occurrence(orchestrator.GAP_MARKETS, '2026-09-23T02:00:00Z')
        with self.assertRaises(orchestrator.InventedMarketRefused):
            orchestrator.invent_open_ticker(orchestrator.PANEL_ABSENT_FROM_REGET[0], 'yes')
        with self.assertRaises(orchestrator.InventedMarketRefused):
            orchestrator.invent_missing_side(orchestrator.ONE_SIDED_EVENTS[0], result='yes')
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.refuse_adversary('list_429_backfill')
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.refuse_adversary('parent_seed_invent')
        with self.assertRaises(orchestrator.InventedMarketRefused):
            orchestrator.refuse_adversary('missing_side_invent')
