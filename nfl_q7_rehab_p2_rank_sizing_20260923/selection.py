"""Frozen B2 selection bar. Fixture arithmetic is not a rehab P&L."""
...
def _decision(status, reason):
    return dict(status=status, reason=reason, candidate_arm=None if status == 'NO_NEW_SELECTION' else 'B2',
...
def select(scenarios):
    """Return the predeclared bar. Does not write a scorecard. Does not invent P&L."""
    ...
    for stress in STRESSES:
        ...
        b2 = _money(scenarios[scenario_name(stress, 'B2')]['completed_strategy_pnl'])
        dee = _money(scenarios[scenario_name(stress, 'D')]['completed_strategy_pnl'])
        if not (b2 > b0):
            return _decision('NO_NEW_SELECTION', 'b2_vs_b0:' + stress)
        if not (dee > 0):
            return _decision('NO_NEW_SELECTION', 'd_positive:' + stress)
        if b2 < Decimal('0.95') * dee:
            return _decision('NO_NEW_SELECTION', 'b2_vs_d:' + stress)
        hours_b0 = _money(scenarios[scenario_name(stress, 'B0')].get('unhedged_contract_hours'))
        hours_b2 = _money(scenarios[scenario_name(stress, 'B2')].get('unhedged_contract_hours'))
        if hours_b0 is None or hours_b2 is None or hours_b2 > Decimal('1.25') * hours_b0:
            return _decision('NO_NEW_SELECTION', 'inventory:' + stress)
    primary = 'q3300_d0.25'
    left = scenarios[scenario_name(primary, 'B0')].get('week_contributions')
    right = scenarios[scenario_name(primary, 'B2')].get('week_contributions')
    ...
    return _decision('B2_SHADOW_CANDIDATE', 'bar_passed_on_supplied_rows')
