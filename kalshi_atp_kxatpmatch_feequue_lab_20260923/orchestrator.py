...
The checkout freeze, scout hunt, panel stub, and accept stamp match the
attached sha256 values. Panel markets are the scout-hunt objects. Lee-Ready
is refused. Fill density and occurrence_datetime are not invented. The NFL
000 path is a pointer only.
"""
import hashlib
...
PANEL_PURPOSE = 'GET-only ATP match ML fee+queue honesty harness seed from Scout hunt bytes'
STUBBED_AT_ET = '2026-09-23T14:12:00-04:00'
...
CONDUCTOR_ACCEPT_SHA256 = '93aad226541bd41974dfe714f8d17c05b8ab0cc0d2259627c217f446af83b596'
EXAMINER_HOLD_SHA256 = '503ed0e734fe16a87a989193f56f2a61f83a2d123713504795bee8da6e8cdb82'
SOURCE_PINS_SHA256 = '2d652c808209f5c1086e20e2cdfb8c0cf16049401172435a35bde209a96876ea'
FREEZE_BYTES = 4893
SCOUT_BYTES = 112834
PANEL_BYTES = 35140
STAMP_BYTES = 498
...
COMMIT_VERBATIM = ('freeze', 'scout_hunt', 'panel_stub')
...
class InventedFillDensityRefused(OrchestratorError):
    """Fill density is not invented from volume or from a size quote."""
...
class InventedSoTRefused(OrchestratorError):
    """occurrence_datetime is not invented or rewritten."""
...
def assign_fill_density(market, value=None):
    """Volume and size quotes do not become a fill density."""
    del market, value
    raise InventedFillDensityRefused()


def assign_occurrence(market, value=None):
    """Authentic occurrence_datetime stays on the committed bytes."""
    del market, value
...
def _refuse_occurrence(row):
    """A classification row does not supply or rewrite occurrence_datetime."""
...
def scout_index():
    ...
    for market in markets:
        ...
        if 'fill_density' in market:
            raise InventedFillDensityRefused()
        occurrence = _occurrence_token(market.get('occurrence_datetime'))
        prior = event_occurrence.get(event)
...
def _assert_market_object(market, index):
    ...
    if ticker not in index['by_ticker']:
        raise InventedMarketRefused()
    authentic = index['by_ticker'][ticker]
    if market.get('occurrence_datetime') != authentic.get('occurrence_datetime'):
        raise InventedSoTRefused()
    if 'fill_density' in market:
        raise InventedFillDensityRefused()
    if market != authentic:
...
def _assert_source_pins(payload):
    ...
    if payload.get('panel_subset_of_scout') is not True:
        raise OrchestratorError('source pins')
    if payload.get('panel_scout_subset') is not True:
        raise OrchestratorError('source pins')
    ...
    if payload.get('nhl_fq_reopen') is not False or payload.get('cpi_fq_reopen') is not False:
        raise OrchestratorError('source pins')
    if payload.get('nearest_dead_card') != NEAREST_DEAD_CARD:
        raise OrchestratorError('source pins')
    for key in OUTPUT_KEYS:
...
def conductor_pin_status():
    ...
    accept_match = sha256_file(CONDUCTOR_ACCEPT) == CONDUCTOR_ACCEPT_SHA256
    return {
        ...
        'conductor_bytes_in_checkout': freeze_match and scout_match and stub_match and accept_match,
...
def _assert_authentic_bytes():
    ...
    if accept.get('panel_version') != PANEL_VERSION:
        raise OrchestratorError('conductor accept')
    if accept.get('panel_scout_subset') is not True or accept.get('admitted_at') is not None:
        raise OrchestratorError('conductor accept')
    if accept.get('feature_family') != FEATURE_FAMILY or accept.get('packet_id') != EXPERIMENT_ID:
        raise OrchestratorError('conductor accept')
    if accept.get('lab_dir') != LAB_DIRECTORY + '/':
        raise OrchestratorError('conductor accept')
    if accept.get('nearest_dead_card') != NEAREST_DEAD_CARD:
        raise OrchestratorError('conductor accept')
    if accept.get('commit_verbatim') != list(COMMIT_VERBATIM):
        raise OrchestratorError('conductor accept')
    integrity = accept.get('integrity')
    if not isinstance(integrity, dict):
        raise OrchestratorError('conductor accept')
    if integrity.get('panel_markets_byte_equal_scout_objects') is not True:
        raise OrchestratorError('conductor accept')
    if integrity.get('panel_events_subset_of_scout') is not True:
        raise OrchestratorError('conductor accept')
    if integrity.get('capture_alias_identical') is not True:
        raise OrchestratorError('conductor accept')
    if integrity.get('freeze_copies_identical') is not True:
        raise OrchestratorError('conductor accept')
    ...
    if hold.get('feature_family') != FEATURE_FAMILY or hold.get('packet') != EXPERIMENT_ID:
        raise OrchestratorError('examiner hold')
...
def _validate_panel(payload, admitted):
    ...
    if payload.get('purpose') != PANEL_PURPOSE:
        raise OrchestratorError('purpose')
    if payload.get('stubbed_at_et') != STUBBED_AT_ET:
    ...
    for event in events:
        ...
        if occurrence != index['event_occurrence'].get(name):
            raise InventedSoTRefused()
...
def instrument_binding(panel=None):
    ...
    return {
        ...
        'nearest_dead_card': NEAREST_DEAD_CARD,
        'fee_is_knob': False,
