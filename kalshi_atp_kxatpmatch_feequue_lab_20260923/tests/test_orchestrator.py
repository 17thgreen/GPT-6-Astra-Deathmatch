"""Unit pins for the ATP KXATPMATCH fee and queue honesty harness.
...
class PinTests(unittest.TestCase):
    def test_binding_pins_the_seed_and_the_instruments(self):
        ...
        self.assertEqual(binding['experiment_id'], 'ATP-KXATPMATCH-FEEQUEUE-HARNESS')
        self.assertEqual(binding['feature_family'], 'ATP-FQ')
        self.assertEqual(frozen['feature_family'], 'ATP-FQ')
        self.assertEqual(stamp['feature_family'], 'ATP-FQ')
        self.assertEqual(binding['knob'], 'analysis_slice')
        self.assertEqual(frozen['knob'], 'analysis_slice')
        self.assertEqual(stamp['packet_id'], 'ATP-KXATPMATCH-FEEQUEUE-HARNESS')
        self.assertEqual(binding['panel_version'], '2026-09-23.atp-kxatpmatch-v0')
        ...
        self.assertEqual(binding['scout_markets_n'], 48)
        self.assertEqual(binding['scout_events_n'], 24)
        self.assertEqual(len(panel['events']), 6)
        self.assertEqual(len(panel['markets']), 12)
        self.assertIs(binding['panel_subset_of_scout'], True)
        self.assertIs(binding['panel_scout_subset'], True)
        ...
        self.assertEqual(
            binding['nearest_dead_card'],
            'invent_depth_fills_fill_density_OR_invent_occurrence_datetime_SoT',
        )
        ...
        self.assertEqual(binding['conductor_accept_sha256'], orchestrator.CONDUCTOR_ACCEPT_SHA256)
        ...
        self.assertEqual(
            ...
            '93aad226541bd41974dfe714f8d17c05b8ab0cc0d2259627c217f446af83b596',
        )
        ...
        self.assertEqual(frozen['conductor_accept_sha256'], orchestrator.CONDUCTOR_ACCEPT_SHA256)
        self.assertIsNone(frozen['results'])
        self.assertIsNone(frozen['pnl'])
        self.assertIsNone(stamp['results'])
        self.assertIsNone(stamp['pnl'])
        self.assertEqual(binding['base_commit'], '6e55a99790f4cc09a437d2e16ccf4e8e826a154b')
    ...
    def test_seed_loads_and_admitted_panel_is_preferred_when_present(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stamped['panel_version'] = '2026-09-23.atp-kxatpmatch-v1'
    ...
    def test_panel_markets_are_the_scout_subset(self):
        ...
        self.assertEqual(index['markets_n'], 48)
        self.assertEqual(index['events_n'], 24)
        self.assertEqual(len(panel['markets']), 12)
        ...
        for event in panel['events']:
            ...
            self.assertEqual(
                event['occurrence_datetime'],
                index['event_occurrence'][event['event_ticker']],
            )
            self.assertTrue(event['occurrence_datetime'].endswith('Z'))
    ...
    def test_source_pins_and_pre_accept_empty(self):
        ...
        self.assertEqual(pins['conductor_accept_sha256'], orchestrator.CONDUCTOR_ACCEPT_SHA256)
        self.assertEqual(pins['governance_tree'], 'absent')
        self.assertIs(pins['panel_subset_of_scout'], True)
        self.assertIs(pins['panel_scout_subset'], True)
        ...
        self.assertEqual(accept['post_cpifq_main'], orchestrator.BASE_COMMIT)
        self.assertIs(accept['panel_scout_subset'], True)
        self.assertIsNone(accept['admitted_at'])
        self.assertIsNone(accept['results'])
        self.assertIsNone(accept['pnl'])
...
class SchemaTests(unittest.TestCase):
    def test_atpa0_and_atpa1_schema_on_the_panel_stub(self):
        before = {path: path.read_bytes() for path in _freeze_paths()}
        native = orchestrator.conduct(orchestrator.ATPA0)
        fresh = orchestrator.conduct(orchestrator.ATPA1)
        ...
        self.assertEqual(native['scout_markets_n'], 48)
        self.assertEqual(fresh['scout_events_n'], 24)
    ...
    def test_native_partition_and_fresh_bins_stay_out_of_the_freeze(self):
        ...
        self.assertEqual(native['arm'], 'ATPA0')
        ...
        self.assertEqual(fresh['arm'], 'ATPA1')
        ...
        with self.assertRaises(orchestrator.UnknownSlice):
            orchestrator.conduct('ATPA2')
        for path, raw in before.items():
            self.assertEqual(path.read_bytes(), raw)

    def test_atpa1_ignores_taker_side_when_binning(self):
        base = {
            'queue_ahead': '3300',
            'previous': None,
...
class RefuseTests(unittest.TestCase):
    ...
    def test_scout_ticker_depth_density_and_occurrence_are_refused(self):
        panel = orchestrator.load_panel()
        ...
        with self.assertRaises(orchestrator.InventedFillDensityRefused):
            orchestrator.classify_native_taker({
                'ticker': 'KXATPMATCH-SYN-DENSITY',
                'taker_outcome_side': 'yes',
                'taker_book_side': 'bid',
                'fill_density': '1.5',
            })
        with self.assertRaises(orchestrator.InventedSoTRefused):
            orchestrator.assign_occurrence(market, '2026-01-01T00:00:00Z')
        with self.assertRaises(orchestrator.InventedSoTRefused):
            orchestrator.classify_fresh_queue({
                'row_id': 'rewritten-sot',
                'queue_ahead': '3300',
                'current': {'yes_bid': '0.40'},
                'transaction_time': '1',
                'occurrence_datetime': '2026-01-01T00:00:00Z',
            })
        cloned = dict(market)
        ...
        with self.assertRaises(orchestrator.InventedMarketRefused):
            orchestrator._assert_market_object({
                'ticker': 'KXATPMATCH-SYN-NOT-IN-HUNT',
                'event_ticker': 'KXATPMATCH-SYN',
                'status': 'active',
                'result': '',
