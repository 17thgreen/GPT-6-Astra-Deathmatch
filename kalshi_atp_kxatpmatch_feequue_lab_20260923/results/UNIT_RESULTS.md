...
Ran 12 tests in 0.061s at 2026-09-23T18:24:02Z. Result: OK. Failures: 0.
Errors: 0.
...
Feebook, rails, Q, Cap-SR, Cap-SR-FX, C1–C5, S4, S5, NHL-FQ, CPI-FQ, L2-*,
EMPTY-OB, and SOT-ID match
...
- accept stamp `93aad226541bd41974dfe714f8d17c05b8ab0cc0d2259627c217f446af83b596`
- `SOURCE_PINS.json` sha256 `2d652c808209f5c1086e20e2cdfb8c0cf16049401172435a35bde209a96876ea`

`conductor_pin_status` reports the freeze, scout, panel, and accept matches
and `conductor_bytes_in_checkout` true. `results` and `pnl` stay null. The
2026-09-23T18:24:02Z unit run is code verification. It is not an Examiner
...
- ATPA0 and ATPA1 on the conductor stub report `event_count` 6 and
...
- A scout ticker passed in as a native trade is refused. Public
  `yes_bid_size_fp` on the stub is not a depth ladder and is not a fill
  density. `quote_depth` and `assign_fill_density` refuse. `n_books` stays
  null.
- Every scout market and every panel event already carries an
  `occurrence_datetime` ending in `Z`. That value is read from the
  committed bytes. `assign_occurrence` refuses a rewrite. A classification
  row that supplies `occurrence_datetime` is refused.
...
- This packet does not ungate S1, S2, or R2-P4. It does not reopen NHL-FQ
  or CPI-FQ.
