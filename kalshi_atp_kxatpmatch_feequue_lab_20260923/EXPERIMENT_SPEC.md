...
`SOURCE_PINS.json` lists the authentic digests, including the accept stamp.
...
| ATPA1 | `content_fresh_vs_stale_bin` — rails `content_fresh_flag` and queue-attribution bins only. No fee invent. No invented fills |
...
No live orders. No Logan keys. No invented markets, depth, fills,
fill-density, `occurrence_datetime`, or PnL. No Lee-Ready. No `admit.py`.
No Q6-`000` retune. No Cap-SR, QF, L2, EMPTY-OB, SOT-ID, L2-SF, NHL-FQ, or
CPI-FQ reopen. This packet does not ungate S1, S2, or R2-P4. ATL@GB stays
refused. A later unit page is code verification only, not an Examiner score.
Nearest dead card: inventing depth, fills, or fill-density, or inventing
`occurrence_datetime` / SoT. Authentic panel occurrence timestamps stay as
committed. They are not rewritten and they are not a score.
