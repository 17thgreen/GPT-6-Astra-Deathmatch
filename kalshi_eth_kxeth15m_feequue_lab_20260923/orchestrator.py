"""ETH KXETH15M fee and queue honesty harness.
...
The checkout freeze, scout hunt, panel stub, and accept match the attached
sha256 values. The panel is the full tiny hunt. A missing
occurrence_datetime stays missing. Fill density is not invented.
Lee-Ready is refused. The NFL 000 path is a pointer only. C5 and ATP-FQ
stay closed. This module does not trade crypto.
"""
import hashlib
...
LAB_DIRECTORY = 'kalshi_eth_kxeth15m_feequue_lab_20260923'
EXPERIMENT_ID = 'ETH-KXETH15M-FEEQUEUE-HARNESS'
FEATURE_FAMILY = 'ETH-FQ'
SERIES = 'KXETH15M'
SCHEMA_ID = 'astra.panel_stub.v0'
PANEL_VERSION = '2026-09-23.eth-kxeth15m-v0'
COHORT_KIND = 'scout_hunt_full_tiny'
STUB_STATUS = 'NOT_ADMITTED'
PURPOSE = 'GET-only ETH 15m fee+queue honesty harness seed from Scout hunt bytes (C5 sibling leftover)'
STUBBED_AT = '2026-09-23T14:29:19-04:00'
SCOUT_CITE = 'packets/scout_eth_kxeth15m_2026-09-23/scout_hunt_KXETH15M.json'
KNOB = 'analysis_slice'
ETHA0 = 'ETHA0'
ETHA1 = 'ETHA1'
ARMS = (ETHA0, ETHA1)
...
BASE_COMMIT = '438f4abf28a3c0156daf6c96ece5efda9557f1dc'
FREEZE_SHA256 = '9cae3bad089e6e18bee22694a36a1a2c18db33935a315766cd6bd8f8476aa83a'
SCOUT_SHA256 = '18f70001c8d68418d435e2016b756f90753e374a92d323f8e999f76215d9cf9c'
PANEL_STUB_SHA256 = 'b3379783c84eaa910f6a57f5318b8536f21220cfeaf0f73e9ff73ee0f20dd90d'
CONDUCTOR_STAMP_SHA256 = '1e132317ca65488505dac464d660fc9bfeb9517c9b994bd2a3553f9eb316e3d3'
PRE_ACCEPT_EMPTY_SHA256 = '88395e7bba510262bcac68565dbacc82ec52400335f5528135bd50fdaa06925b'
CONDUCTOR_ACCEPT_SHA256 = '3e553395a47d5ced1a4b48d81b8d0d760d984becdbcb6c1dd574dafc39621a5e'
EXAMINER_HOLD_SHA256 = '070826fe583bc8c0a91c689a31bb956ebd14349f098b8a6a15776f16dfe4ce09'
SOURCE_PINS_SHA256 = '087de494e176e8a8abb20b39eac962bdc5177f4336fcdeed66d5ea0c9871d810'
FREEZE_BYTES = 5431
SCOUT_BYTES = 2148
PANEL_BYTES = 4147
STAMP_BYTES = 496
...
FEE_CHANNEL_CITE = 'quadratic'
FEE_MULTIPLIER = 1
ACCEPT_STAMPED_AT = '2026-09-23T14:41:00-04:00'
SERIES_FEE_CHANNEL = {
    'fee_type': 'quadratic',
    'fee_multiplier': 1,
    'source': 'GET /series/KXETH15M',
    'note': 'same quadratic channel as C5 KXBTC15M; market objects lack fee_* fields',
}
ACCEPT_HARD_REFUSE = (
    'invent_fills',
    'invent_depth',
...
PANEL_RULES = (
    ...
    'no_C5_or_ATP_reopen',
    'no_bacchus_or_kxeth15m_strategy_port',
    'not_live_crypto_trading',
)
COHORT_SUMMARY = {
    ...
    'subset_note': 'hunt tiny (1 market / 1 event) — panel uses ALL authentic open markets; no invent',
}
...
ADVERSARY_LABELS = {
    ...
    'c5_reopen': 'C5 reopen is refused',
    ...
    'r3_p1': 'R3-P1 stays out of this harness',
    'r3_p2': 'R3-P2 stays out of this harness',
...
FREEZE_NAME = 'ETH_KXETH15M_FEEQUEUE_HARNESS_FREEZE_2026-09-23.md'
SCOUT_NAME = 'scout_hunt_KXETH15M.json'
PANEL_ALIAS = 'ETH_KXETH15M_PANEL_STUB_2026-09-23.json'
STAMP_NAME = 'CONDUCTOR_FROZEN_EXPERIMENT.json'
PRE_ACCEPT_NAME = 'PRE_ACCEPT_EMPTY_RESULTS.json'
ACCEPT_NAME = 'CONDUCTOR_ACCEPT_ETH_KXETH15M_FEEQUEUE_HARNESS_2026-09-23.json'
HOLD_NAME = 'EXAMINER_HOLD_ETH_KXETH15M_FEEQUEUE_HARNESS_PRE_PR_2026-09-23.json'
SOURCE_PINS_NAME = 'SOURCE_PINS.json'
PACKET = ROOT / FREEZE_NAME
LAB_BUNDLE = ROOT / 'ETH_KXETH15M_FEEQUEUE_HARNESS'
GOVERNANCE_BUNDLE = PARENT / 'packets' / 'ETH_KXETH15M_FEEQUEUE_HARNESS'
GOVERNANCE_TREE = PARENT / 'lab' / 'governance' / 'astra' / 'packets'
SCOUT_CANONICAL = PARENT / 'packets' / 'scout_eth_kxeth15m_2026-09-23' / SCOUT_NAME
SCOUT_ALIAS = PARENT / 'packets' / 'scout_cashcow_hunt_2026-09-22' / SCOUT_NAME
...
PANEL_STUB = PARENT / 'lab' / 'astra-capture' / 'eth-kxeth15m' / 'panel_stub.json'
PANEL_ADMITTED = PARENT / 'lab' / 'astra-capture' / 'eth-kxeth15m' / 'panel_admitted.json'
...
class StrategyPortRefused(OrchestratorError):
    """No bacchus or kxeth15m strategy is loaded."""

    def __init__(self):
        super().__init__('strategy port refused')
...
class PanelVersionRefused(OrchestratorError):
    """The panel file is not the pinned ETH seed."""
...
def _load_hygiene():
    spec = importlib.util.spec_from_file_location('eth_kxeth15m_hygiene', HYGIENE_PATH)
    if spec is None or spec.loader is None:
        raise OrchestratorError('hygiene import')
    module = importlib.util.module_from_spec(spec)
    sys.modules['eth_kxeth15m_hygiene'] = module
...
def port_strategy(name=None):
    """No bacchus or kxeth15m strategy is loaded from this harness."""
    del name
...
def refuse_adversary(label):
    ...
    if label == 'claim_s1_green':
        raise S1GreenClaimRefused()
    if label in ('bacchus_port', 'kxeth15m_strategy_port'):
        raise StrategyPortRefused()
    if label == 'live_crypto':
        raise LiveOrdersForbidden()
...
def scout_index():
    """Authentic hunt: 1 market, 1 event. Cached after the digest matches."""
    global _SCOUT_INDEX
...
def _assert_source_pins(payload):
    ...
    if payload.get('panel_full_scout') is not True:
        raise OrchestratorError('source pins')
    if payload.get('series_fee_type') != 'quadratic' or payload.get('series_fee_multiplier') != FEE_MULTIPLIER:
        raise OrchestratorError('source pins')
    if payload.get('series_fee_source') != 'GET /series/KXETH15M':
        raise OrchestratorError('source pins')
    if payload.get('fee_override_applied') is not False:
        raise OrchestratorError('source pins')
    if payload.get('scout_hunt_alias_path') != 'packets/scout_cashcow_hunt_2026-09-22/scout_hunt_KXETH15M.json':
        raise OrchestratorError('source pins')
    ...
    if payload.get('nhl_fq_reopen') is not False or payload.get('cpi_fq_reopen') is not False:
        raise OrchestratorError('source pins')
    if payload.get('c5_reopen') is not False or payload.get('atp_fq_reopen') is not False:
        raise OrchestratorError('source pins')
    if payload.get('bacchus_strategy_port') is not False or payload.get('kxeth15m_strategy_port') is not False:
        raise StrategyPortRefused()
    if payload.get('r3_p1_in_scope') is not False or payload.get('r3_p2_in_scope') is not False:
        raise OrchestratorError('source pins')
    if payload.get('live_crypto_trading') is not False:
        raise LiveOrdersForbidden()
...
def _assert_authentic_bytes():
    ...
    _assert_digest(SCOUT_ALIAS, SCOUT_SHA256, SCOUT_BYTES)
    ...
    if stamp.get('results') is not None or stamp.get('pnl') is not None:
        raise ScorecardPromotionRefused()
    if stamp.get('stamped_at_et') != STUBBED_AT:
        raise OrchestratorError('conductor stamp')
    ...
    if accept.get('decision') != 'ACCEPT' or accept.get('implement') is not True:
        raise OrchestratorError('conductor accept')
    if accept.get('seat') != 'Conductor':
        raise OrchestratorError('conductor accept')
    if accept.get('accepted_at_et') != ACCEPT_STAMPED_AT:
        raise OrchestratorError('conductor accept')
    digest = accept.get('digest_verify')
    if not isinstance(digest, dict):
        raise OrchestratorError('conductor accept')
    if digest.get('panel_stub_sha256') != PANEL_STUB_SHA256:
        raise OrchestratorError('conductor accept')
    if digest.get('scout_hunt_KXETH15M_sha256') != SCOUT_SHA256:
        raise OrchestratorError('conductor accept')
    if digest.get('panel_stub_match') is not True or digest.get('hunt_match') is not True:
        raise OrchestratorError('conductor accept')
    if accept.get('hard_refuse') != list(ACCEPT_HARD_REFUSE):
        raise OrchestratorError('conductor accept')
    note = accept.get('notes')
    if not isinstance(note, str) or 'R3-P2/R3-P1' not in note or 'do not conflate' not in note:
        raise OrchestratorError('conductor accept')
...
def _validate_panel(payload, admitted):
    ...
    if payload.get('series_fee_channel') != SERIES_FEE_CHANNEL:
        raise OrchestratorError('fee channel')
...
def instrument_binding(panel=None):
    ...
    if panel is None:
        panel = load_panel()
    eth = probe_examiner_fee(SERIES)
    nfl = probe_examiner_fee(NFL_POINTER_SERIES)
    if eth != nfl:
        raise ShadowFeeLiteralRefused()
    channel = panel.get('series_fee_channel')
    if channel != SERIES_FEE_CHANNEL:
        raise OrchestratorError('fee channel')
    ...
    index = scout_index()
    panel_tickers = {market.get('ticker') for market in panel.get('markets') or []}
    if panel_tickers != set(index['by_ticker']):
        raise InventedMarketRefused()
    ...
    return {
        ...
        'series_fee_type': channel['fee_type'],
        'series_fee_multiplier': channel['fee_multiplier'],
        ...
        'probe_formula_id': eth['formula_id'],
        'probe_series_resolution': eth['series_resolution'],
        ...
        'c5_reopen': False,
        'atp_fq_reopen': False,
        'bacchus_strategy_port': False,
        'kxeth15m_strategy_port': False,
        'r3_p1_in_scope': False,
        'r3_p2_in_scope': False,
...
def conduct_native(path=None):
    """ETHA0 schema on synthetic native rows. Does not write the freeze scorecard."""
    rows = partition_native(load_synthetic_trades(path))
    return _report(ETHA0, 'synthetic_schema_standin', {
        'partitions': rows,
...
def conduct_fresh(path=None):
    """ETHA1 schema on synthetic book rows. The gap stays null."""
    rows = partition_fresh(load_synthetic_fresh(path))
    return _report(ETHA1, 'synthetic_schema_standin', {
        'bins': rows,
        'bin_count': len(rows),
    })
