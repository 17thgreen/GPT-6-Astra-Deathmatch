"""Independent fixed-point ledger audit. Does not import the strategy engine."""
import gzip
import json
...
def verify(name, result, market_map):
    ...
    assert config['order_size'] == config['exposure_cap'] == config['assumed_exit_depth'] == 250
    ...
    for f in rows(name, 'fills'):
        ...
        assert cash >= -1e-7 and abs(inventory[event]) <= 250+1e-6
        if f['kind'] == 'maker':
            o = orders[f['order_id']]
            maker += f['size']
...
def main():
    ...
    for g in cohort['games']:
        for m in g['metadata']['markets']:
            ticker = m['ticker']
            market_map[ticker] = dict(event=g['event'], kickoff=g['kickoff'],
                direction=1 if ticker == g['tickers'][0] else -1,
                listed_at=datetime.fromisoformat(m['open_time'].replace('Z', '+00:00')).timestamp())
    summary = json.loads((ROOT / 'results/experiment_summary.json').read_text())
    assert len(summary['scenarios']) == 12 and not summary['failures']
    checks = [verify(n, r, market_map) for n, r in summary['scenarios'].items()]
    (ROOT / 'results/verification.json').write_text(json.dumps(dict(all_passed=True, cases=checks), indent=2))
    print('PASS: 12 independent financial ledgers, fees, inventory, listing, orders and exits')
