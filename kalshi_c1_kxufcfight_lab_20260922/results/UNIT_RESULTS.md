...
Ran 15 tests in 0.003s. Result: OK. Failures: 0. Errors: 0.
...
and retuning `000` raise; the collector stub is `READY` and `clock_admit`,
`mz`, and `roi` raise `ClockRefused`; `lee_ready` raises when a price sits
...
- Settled N is 0. The clock refused admit. `MZ` and `roi` were not
  computed. No live order client was added. No Q1–Q7 directory, and neither
