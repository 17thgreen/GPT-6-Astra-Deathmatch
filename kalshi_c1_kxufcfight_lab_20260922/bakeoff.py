...
The capture panel is ADMITTED. clock_admit accepts that stamp for a fixture
join. results, pnl, MZ, and roi stay null. This module does not place live
orders, does not award fills, and does not invent PnL from settles.
"""
import hashlib
import json
import sys
...
SCHEMA_PANEL_VERSION = '2026-09-22.c1-kxufcfight-meas-v0'
...
SCHEMA_CLOCK = 'NOT_ADMITTED'
SCHEMA_REJOIN = 'KICKED'
SCHEMA_AWAITING = 'CLOCK_ADMIT_PASS'
CLOCK_ADMIT = 'ADMITTED'
PANEL_CLOCK_ADMIT = 'CONDITIONS_MET_COLLECTOR_STAMPED'
ACCEPTED_STAMPS = (
    CLOCK_ADMIT,
    STUB_STATUS,
    PANEL_CLOCK_ADMIT,
)
CLOCK_REJOIN = 'ADMITTED'
REPORTED_SETTLED_N = 4
ADMITTED_SETTLED_N = 0
EVENTS_N = 2
MARKETS_N = 4
EVENTS_FINALIZED = EVENTS_N
SETTLED_N = ADMITTED_SETTLED_N
RESOLUTION_LABEL = 'ADMITTED_FIXTURE_JOIN'
NOT_ADMITTED_LABEL = 'NOT_ADMITTED'
...
CAPTURE_PANEL = PARENT / 'lab' / 'astra-capture' / 'c1-kxufcfight' / 'panel_admitted.json'
FIXTURE_PANEL = ROOT / 'fixtures' / 'panel_admitted.json'
RESOLUTION_CAPTURE_DIR = PARENT / 'lab' / 'astra-capture' / 'c1-kxufcfight' / 'resolutions'
RESOLUTION_FIXTURE = ROOT / 'fixtures' / 'resolution_join_admitted.json'
NOT_ADMITTED_FIXTURE = ROOT / 'fixtures' / 'resolution_hook_not_admitted.json'
...
class ClockRefused(BakeoffError):
    """The stamp is not the pinned ADMITTED panel. The scorecard stays null."""

    def __init__(self):
        super().__init__(
            'stamp is not the pinned ADMITTED panel; '
...
def collector_stub():
    """Ready collector. The panel stamp is ADMITTED. The scorecard is not."""
    stub = {
        'panel_version': PANEL_VERSION,
        'admitted_at': ADMITTED_AT,
        'stub_status': STUB_STATUS,
        ...
        'admitted': True,
        'admitted_settled_panel': True,
        'clock': CLOCK_ADMIT,
        'panel_clock_admit': PANEL_CLOCK_ADMIT,
        'clock_rejoin': CLOCK_REJOIN,
        'settled_n': SETTLED_N,
        'reported_settled_n': REPORTED_SETTLED_N,
        'admitted_settled_n': ADMITTED_SETTLED_N,
...
def _scorecard_payload(mapping):
    if not isinstance(mapping, dict):
        return
    for key in ('pnl', 'roi', 'MZ', 'mz', 'results', 'winner', 'band_roi'):
        if mapping.get(key) is not None:
            raise ScorecardRefused(key)


def load_admitted_panel(path=None):
    """Pinned admitted panel. The fixture copy must match the capture bytes."""
    if not CAPTURE_PANEL.is_file():
        raise BakeoffError('admitted panel')
    canonical_digest = _sha256(CAPTURE_PANEL)
    if canonical_digest != PANEL_SHA256:
        raise BakeoffError('panel sha256')
    fixture_path = FIXTURE_PANEL if path is None else Path(path)
    if _sha256(fixture_path) != PANEL_SHA256:
        raise BakeoffError('fixture panel sha256')
    payload = json.loads(fixture_path.read_text())
    if payload.get('panel_version') != PANEL_VERSION:
        raise BakeoffError('panel_version')
    if payload.get('packet_id') != PACKET_ID:
        raise BakeoffError('packet')
    if payload.get('admitted_at') != ADMITTED_AT:
        raise BakeoffError('admitted_at')
    if payload.get('stub_status') != STUB_STATUS:
        raise BakeoffError('stub_status')
    if payload.get('clock_admit') != PANEL_CLOCK_ADMIT:
        raise ClockRefused()
    gate = payload.get('admit_gate') or {}
    if gate.get('status') != CLOCK_ADMIT:
        raise ClockRefused()
    ...
    if binds.get('fee_lab_sha') != FEEBOOK_COMMIT:
        raise BakeoffError('feebook commit')
    if binds.get('rails_lab_sha') != RAILS_COMMIT:
        raise BakeoffError('rails commit')
    ...
    if counts.get('events_n') != EVENTS_N or len(events) != EVENTS_N:
        raise BakeoffError('events')
    if counts.get('markets_n') != MARKETS_N or len(markets) != MARKETS_N:
        raise BakeoffError('markets')
    return payload


def clock_admit(stamp=CLOCK_ADMIT, panel_path=None):
    """Accept the pinned ADMITTED stamp. Do not fill the scorecard."""
    if stamp not in ACCEPTED_STAMPS:
        raise ClockRefused()
    panel = load_admitted_panel(panel_path)
    record = {
        'clock_admit': CLOCK_ADMIT,
        'stamp': stamp,
        'admit_gate_status': panel['admit_gate']['status'],
        'stub_status': STUB_STATUS,
        'panel_clock_admit': panel['clock_admit'],
        'panel_version': PANEL_VERSION,
        'admitted_at': ADMITTED_AT,
        'panel_sha256': PANEL_SHA256,
        'admitted': True,
        ...
        'markets_n': MARKETS_N,
        'fixture_join': True,
        'scorecard_filled_from_settles': False,
        'winner': None,
    }
...
def clock_rejoin():
    """The admit stamp consumed the kicked rejoin. The scorecard stays null."""
    admitted = clock_admit()
    wired = wire_resolution_hook()
    record = {
        'clock_rejoin': CLOCK_REJOIN,
        'prior_scaffold_rejoin': SCHEMA_REJOIN,
        'clock_admit': admitted['clock_admit'],
        'panel_version': PANEL_VERSION,
        'admitted_at': ADMITTED_AT,
        'panel_sha256': PANEL_SHA256,
        'reported_settled_n': REPORTED_SETTLED_N,
        ...
        'events_finalized': EVENTS_FINALIZED,
        'admitted': True,
        'resolutions_applied': wired['resolutions_applied'],
        'scorecard_filled_from_settles': False,
        'winner': None,
    }
    record.update(_null_metrics())
...
def apply_resolutions(rows=None, **kwargs):
    """Join the pinned fixture only. Caller settles and payoffs are refused."""
    _scorecard_payload(kwargs)
    if kwargs:
        raise ClockRefused()
    if rows is not None:
        ...
        for row in rows:
            if isinstance(row, dict):
                _scorecard_payload(row)
        raise ClockRefused()
    return wire_resolution_hook()


def mz(*_args, **_kwargs):
    """Mincer-Zarnowitz stays null after the admit stamp."""
    raise ScorecardRefused('MZ stays null; the admit stamp is not a statistic')


def roi(*_args, **_kwargs):
    """Return stays null after the admit stamp."""
    raise ScorecardRefused('roi stays null; the admit stamp is not a return')


def write_scorecard(*_args, **_kwargs):
    """Refuse every scorecard fill. The admit stamp is not an Examiner score."""
    raise ScorecardRefused(
        'ADMITTED stamp does not fill results, pnl, MZ, or roi'
    )
...
def load_schema_fixture(path=None):
    ...
    if payload.get('panel_version') != SCHEMA_PANEL_VERSION:
        raise ValueError('panel_version')
    if payload.get('collector') != COLLECTOR_STATUS:
        raise ValueError('collector')
    if payload.get('clock') != SCHEMA_CLOCK:
        raise ValueError('clock')
    if payload.get('clock_rejoin') != SCHEMA_REJOIN:
        raise ValueError('clock_rejoin')
    if payload.get('awaiting') != SCHEMA_AWAITING:
        raise ClockRefused()
...
def walk_schema(path=None, table=None):
    ...
    walked = {
        ...
        'collector': COLLECTOR_STATUS,
        'clock': SCHEMA_CLOCK,
        'clock_rejoin': SCHEMA_REJOIN,
        'awaiting': SCHEMA_AWAITING,
        'settled_n': SETTLED_N,
        'reported_settled_n': REPORTED_SETTLED_N,
        'admitted_settled_n': ADMITTED_SETTLED_N,
        'events_finalized': EVENTS_FINALIZED,
...
def resolution_capture_status():
    """The cited resolutions directory. Absent bytes stay absent."""
    if not RESOLUTION_CAPTURE_DIR.is_dir():
        names = []
    else:
        names = sorted(path.name for path in RESOLUTION_CAPTURE_DIR.glob('*.json'))
    return {
        'path': 'lab/astra-capture/c1-kxufcfight/resolutions',
        'cited_file': 'CONGUA_resolution_rows_latest.json',
        'bytes_in_checkout': bool(names),
        'files': names,
    }


def load_resolution_hook(path=None, panel=None):
    """Join admitted market result labels. A payoff on a row is refused."""
    if path is None:
        path = RESOLUTION_FIXTURE
    payload = json.loads(Path(path).read_text())
    if payload.get('packet_id') != PACKET_ID:
        raise ValueError('packet_id')
    if payload.get('label') == NOT_ADMITTED_LABEL:
        raise ClockRefused()
    ...
    if payload.get('panel_version') != PANEL_VERSION:
        raise ValueError('panel_version')
    if payload.get('admitted_at') != ADMITTED_AT:
        raise ValueError('admitted_at')
    if payload.get('panel_sha256') != PANEL_SHA256:
        raise BakeoffError('panel sha256')
    if payload.get('events_n') != EVENTS_N or payload.get('markets_n') != MARKETS_N:
        raise ValueError('cohort')
    capture = resolution_capture_status()
    if payload.get('resolution_capture_bytes_in_checkout') is not False:
        raise BakeoffError('resolution capture bytes')
    if capture['bytes_in_checkout'] is not False:
        raise BakeoffError('resolution capture bytes')
    ...
    if payload.get('winner') is not None:
        raise ScorecardRefused('winner')
    if panel is None:
        panel = load_admitted_panel()
    by_ticker = {row['market_ticker']: row for row in panel['markets']}
    rows = payload.get('rows')
    if not isinstance(rows, list) or len(rows) != MARKETS_N:
        raise ValueError('rows')
    seen = set()
    for row in rows:
        if not isinstance(row, dict):
        ...
        if ticker not in by_ticker or ticker in seen:
            raise ValueError('market_ticker')
        seen.add(ticker)
        source = by_ticker[ticker]
        if row.get('event_ticker') != source.get('event_ticker'):
            raise ValueError('event_ticker')
        if row.get('result_observed_live_get') != source.get('result_observed_live_get'):
            raise BakeoffError('result')
        if row.get('result_observed_live_get') not in ('yes', 'no'):
            raise BakeoffError('result')
        if row.get('settlement_ts') != source.get('settlement_ts'):
            raise ValueError('settlement_ts')
        if row.get('status_observed') != 'finalized':
            raise ValueError('status_observed')
        _scorecard_payload(row)
    if seen != set(by_ticker):
        raise ValueError('markets')
    return payload


def wire_resolution_hook(path=None):
    """Apply the admitted result labels. Do not write a payoff."""
    ...
    wired = {
        ...
        'markets_n': MARKETS_N,
        'events_finalized': EVENTS_FINALIZED,
        'market_tickers': [row['market_ticker'] for row in payload['rows']],
        'result_labels': [
            row['result_observed_live_get'] for row in payload['rows']
        ],
        'resolutions_applied': len(payload['rows']),
        'resolution_capture_bytes_in_checkout': False,
        'payoff_computed': False,
        'admitted': True,
        'admitted_settled_panel': True,
        'scorecard_filled_from_settles': False,
        'winner': None,
...
def empty_outputs():
    """The empty-file contract. An admit join does not fill it."""
    payload = {
        'settled_n': SETTLED_N,
        'reported_settled_n': REPORTED_SETTLED_N,
        'admitted_settled_n': ADMITTED_SETTLED_N,
