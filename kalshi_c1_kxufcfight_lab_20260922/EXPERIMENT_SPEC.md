...
September 23, 2026. This file is the hypothesis. The admit-wire correction
below is committed before that wiring's unit outcome is recorded. No figure
in this document is a Kalshi trading result. Q1–Q6 outcomes that already
exist are not re-labeled as evidence from this lab.
...
## Admit stamp — hypothesis before the wiring outcome

Closed draft PR 17 (`cursor/c1-kxufcfight-meas-scaffold-63a7`) encoded
`NOT_ADMITTED` and was closed without merge. The capture panel on main is
already admitted.
...
| `admit_gate.status` | `ADMITTED` |
| panel `clock_admit` | `CONDITIONS_MET_COLLECTOR_STAMPED` |
| path | `lab/astra-capture/c1-kxufcfight/panel_admitted.json` |
| sha256 | `24426d804c51bde23cf2557a11a8481a12026da10024094c4ae546d1f7d3956e` |
| events | 2 |
| markets | 4 |

The cited capture directory
`lab/astra-capture/c1-kxufcfight/resolutions/` is absent from this checkout.
The panel names
`resolutions/CONGUA_resolution_rows_latest.json`. Those bytes are not in
git. The fixture join uses the four `result_observed_live_get` values
already stored on the admitted panel markets. It does not reconstruct the
missing capture file and it does not turn a yes or no into a payoff.

`clock_admit` accepts the stamp `ADMITTED` (and the same panel's
`PANEL_ADMITTED_C1_UFC_ONLY` and `CONDITIONS_MET_COLLECTOR_STAMPED` tokens)
when the fixture copy matches that sha256. The string `ADMIT_PASS` stays
refused. `mz`, `roi`, and `write_scorecard` stay refused. `results`, `pnl`,
`MZ`, and `roi` on `FROZEN_EXPERIMENT.json` stay null. A joined yes or no
is not profit.
...
Schema sheet `panel_version`: `2026-09-22.c1-kxufcfight-meas-v0`.

Admitted panel `panel_version`: `2026-09-22.c1-kxufcfight-v0`.

The schema sheet stays `NOT_ADMITTED`. It is not the admitted panel.
`clock_admit` on the pinned panel stamp returns `ADMITTED` and does not
fill the scorecard. An Examiner pass is not this scaffold. `MZ` and `roi`
have no successful path after the stamp.

The original scaffold recorded Clock `REFUSED` at admitted settled N 0, then
`NOT_ADMITTED` after a kicked rejoin. That encoding is the closed PR. The
admit-stamp section replaces the live clock status. It does not replace the
null scorecard.
...
## Conductor update — rejoin kicked, later superseded by the admit stamp

September 23, 2026. The closed scaffold recorded both KXUFCFIGHT events
finalized, settled N 4, and clock re-join `KICKED`, with admit
`NOT_ADMITTED`. That file remains as
`fixtures/resolution_hook_not_admitted.json`. It is a negative finding. The
wiring path does not accept it as the live stamp.

The admit-stamp section above replaces that live status. Four market result
labels may be joined from the admitted panel. `results`, `pnl`, `MZ`, and
`roi` stay null. A caller-supplied resolution row, and any pnl, roi, or MZ
on a row, is refused. `apply_resolutions` with no caller payload joins the
pinned fixture only.
...
5. Accept the pinned `ADMITTED` panel stamp for a fixture join of two
   events and four markets, and still leave those four scorecard fields null.
...
stay null after the admit stamp. The stamp is not an Examiner score.
...
`results`, `pnl`, `MZ`, and `roi` stay null after the `ADMITTED` stamp. A
unit file may say that the fixture join named four market result labels
from the admitted panel. It may not fill those scorecard fields from the
settles, and it may not treat a missing `resolutions/` capture file as if
its bytes were present.
