...
def main():
    ...
    summary=read('results/experiment_summary.json');scenarios=summary['scenarios'];assert len(scenarios)==36 and not summary.get('failures')
            ...
            if d['kind']=='allocation':
                ...
                for rank in d['ranks']:
                    ...
                    if rank['has_observed_flow']:
                        assert rank['wait_seconds']>0 and rank['lost_queue_wait_seconds']>=0 and rank['score']>0
                    else:
    ...
    regressions={}
    for old_name,reference in read('Q5_REFERENCES.json').items():
        old=reference['summary'];q=int(old['config']['queue_early']);d=old['config']['order_delay_seconds']
        label='baseline' if old['experiment']=='baseline' else '110' if old_name.endswith('allocation_neutral') else '111'
        new_name=f'q{q}_d{d:g}_{label}';new=scenarios[new_name]
        ...
        hashes={suffix:hashlib.sha256(gzip.decompress((ROOT/'results'/f'{new_name}_{suffix}.jsonl.gz').read_bytes())).hexdigest() for suffix in ['fills','orders']}
        assert hashes==reference['ledger_uncompressed_sha256'],new_name
        regressions[new_name]=dict(differences=differences,exact_ledger_hashes_match=True)
    assert len(regressions)==10
    out=dict(all_checks_passed=True,cases=checked,q5_regressions=regressions,
             caveat='Accounting and exact reference reproduction do not verify historical queue positions or executable fills.')
    (ROOT/'results/verification.json').write_text(json.dumps(out,indent=2));print('PASS all 36 ledgers, per-fill fees, decision budgets, factor controls, frozen hashes and 10 exact Q5 ledger regressions')
