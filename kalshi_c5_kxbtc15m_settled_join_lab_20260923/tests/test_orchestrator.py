"""Unit pins for the C5 KXBTC15M settled-resolution join harness.
...
class PinTests(unittest.TestCase):
    def test_binding_pins_the_seed_and_leaves_the_scorecard_null(self):
        ...
        self.assertFalse(orchestrator.C5_SCOUT_CITE.exists())
        self.assertFalse(orchestrator.C5_PANEL_CITE.exists())
        ...
        self.assertEqual(binding['experiment_id'], 'C5-KXBTC15M-SETTLED-RESOLUTION-JOIN-HARNESS')
        self.assertEqual(binding['feature_family'], 'C5-RJ')
        self.assertEqual(frozen['feature_family'], 'C5-RJ')
        ...
        self.assertEqual(binding['seed_ticker'], 'KXBTC15M-26SEP222045-45')
        self.assertEqual(binding['seed_reget_result'], 'no')
        self.assertIsNone(binding['seed_panel_result'])
        self.assertIs(binding['list_429_backfilled'], False)
        self.assertEqual(binding['finalized_list_429'], '429 too_many_requests')
        self.assertEqual(binding['closed_list_429'], '429 too_many_requests')
        ...
        self.assertIs(binding['not_live_crypto_trading'], True)
        ...
        self.assertIs(binding['c5_honesty_reopen'], False)
        ...
        self.assertIs(pins['governance_c5_cites_present'], False)
    ...
    def test_panel_stub_admitted_at_stays_null(self):
        ...
        panel = orchestrator.load_panel()
        self.assertEqual(panel['packet_id'], 'C5-KXBTC15M-MEAS')
        ...
        for market in panel['markets']:
            ...
            self.assertEqual(market['market_ticker'], orchestrator.SEED_TICKER)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            version['panel_version'] = '2026-09-22.c5-kxbtc15m-v1'
    ...
    def test_reget_honesty_keeps_the_list_429_gaps(self):
        ...
        self.assertEqual(reget['honest_gaps'], {
            'status_finalized_list': '429 too_many_requests',
            'status_closed_list': '429 too_many_requests',
        })
        ...
        self.assertNotIn(orchestrator.OPEN_TICKER, reget['markets'])
        seed_market = reget['markets'][orchestrator.SEED_TICKER]
        self.assertEqual(seed_market['status'], 'finalized')
        self.assertEqual(seed_market['result'], 'no')
        self.assertIsNone(panel['markets'][0]['result'])
        self.assertNotIn('orderbook_fp', seed_market)
        agreed = orchestrator.occurrence_agreement_tickers(panel)
        self.assertEqual(len(agreed), 21)
        self.assertIn(orchestrator.SEED_TICKER, agreed)
        self.assertNotIn(orchestrator.OPEN_TICKER, agreed)
        ...
        self.assertEqual(seed_rows[0]['key'], orchestrator.SEED_TICKER)
        self.assertEqual(seed_rows[0]['result'], 'no')
        self.assertEqual(seed_rows[0]['panel_ticker'], orchestrator.SEED_TICKER)
        ...
        self.assertEqual(sum(1 for row in passes if row['result'] == 'yes'), 10)
        self.assertEqual(sum(1 for row in passes if row['result'] == 'no'), 11)
    ...
    def test_source_pins_and_accept(self):
        ...
        self.assertEqual(pins['governance_c5_cites'], 'absent')
        ...
        self.assertEqual(pins['seed_ticker'], orchestrator.SEED_TICKER)
        self.assertEqual(pins['seed_reget_result'], 'no')
        self.assertIsNone(pins['seed_panel_result'])
        self.assertIs(pins['finalized_closed_429_backfilled'], False)
        ...
        self.assertIs(pins['c5_honesty_reopen'], False)
        ...
        self.assertIn('C5_honesty_reopen', accept['hard_refuse'])
...
class SchemaTests(unittest.TestCase):
    def test_j0_and_j1_schema_leave_the_scorecard_null(self):
        ...
        self.assertEqual(j0['event_count'], 1)
        self.assertEqual(j0['market_count'], 1)
        self.assertEqual(len(j0['row_labels']), 23)
        self.assertEqual(len(j1['occurrence_agreement_tickers']), 21)
        self.assertIn(orchestrator.SEED_TICKER, j1['occurrence_agreement_tickers'])
        self.assertNotIn(orchestrator.OPEN_TICKER, j1['occurrence_agreement_tickers'])
        ...
        for report in (j0, j1):
            ...
            self.assertNotEqual(report['occurrence_match_n'], 21)
...
class RefuseTests(unittest.TestCase):
    def test_lee_ready_live_orders_admit_and_reopens_are_refused(self):
        ...
        with self.assertRaises(orchestrator.LiveOrdersForbidden):
            orchestrator.refuse_adversary('live_crypto_trading')
    ...
    def test_invented_result_depth_fills_and_list_429_backfill_are_refused(self):
        panel = orchestrator.load_panel()
        reget = orchestrator.load_reget()
        market = reget['markets'][orchestrator.SEED_TICKER]
        ...
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.invent_result(orchestrator.SEED_TICKER, 'yes')
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.backfill_list_429(
                'status_finalized_list',
                result='yes',
                occurrence_datetime='2026-09-23T19:20:00Z',
            )
        with self.assertRaises(orchestrator.InventedSoTRefused):
            orchestrator.assign_occurrence('status_closed_list', '2026-09-23T19:20:00Z')
        with self.assertRaises(orchestrator.InventedMarketRefused):
            orchestrator.invent_open_ticker(orchestrator.OPEN_TICKER, 'yes')
        with self.assertRaises(orchestrator.InventedResultRefused):
