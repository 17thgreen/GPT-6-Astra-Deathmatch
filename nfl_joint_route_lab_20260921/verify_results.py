...
def main():
    ...
    for name,digest in json.loads((INPUTS/'manifest.json').read_text())['sha256'].items():assert hashlib.sha256((INPUTS/name).read_bytes()).hexdigest()==digest,name
    summary=read('results/experiment_summary.json');scenarios=summary['scenarios'];assert len(scenarios)==12 and not summary.get('failures')
    markets=json.loads((INPUTS/'markets.json').read_text());weeks=json.loads((INPUTS/'week_membership.json').read_text());checked=[]
    ...
    regressions={}
    for name,reference in read('Q7_REFERENCES.json').items():
    ...
    for name,result in scenarios.items():
        ...
        for d in rows(name,'pairs'):
            ...
            if result['arm'] in ('router_on','rescue','joint') and not passed:
    ...
    out=dict(all_checks_passed=True,cases=checked,q7_regressions=regressions,guard_diagnostics=guard_diagnostics,
             caveat='Accounting and exact reference reproduction do not verify historical queue positions or executable fills.')
    (ROOT/'results/verification.json').write_text(json.dumps(out,indent=2));print('PASS all 12 ledgers, per-fill fees, guard controls, frozen hashes and 4 exact Q7 ledger regressions')
