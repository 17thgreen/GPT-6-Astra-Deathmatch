"""Restartable public GET recorder. SQLite stores raw responses and atomic watermarks.

No credentials, order routes, or execution methods. Finite sessions can be
restarted by a durable host's supervisor; do not infer hosting from this file.
"""
import argparse,fcntl,hashlib,json,math,os,re,signal,sqlite3,time
...
def get(path,params):
    ...
    for _ in range(2):
        started=time.time()
        try:
            with urlopen(Request(BASE+path+'?'+urlencode(params),headers={'User-Agent':'NFLTimingResearch/4.0'}),timeout=20) as r:body=json.load(r)
...
def validate_panel(panel):
    if panel.get('purpose')!='development':raise ValueError('This recorder only admits an explicit development panel; use a separately reviewed holdout collection configuration')
    events=panel.get('events',[])
    if not events or len({g['event'] for g in events})!=len(events):raise ValueError('Missing or duplicate events')
    for g in events:
        if not allowed('events/'+g['event']):raise ValueError('Invalid event identifier')
        datetime.fromisoformat(g['kickoff'])
    return events
...
def run(panel_path,database,seconds=86400,interval=5):
    ...
    panel_bytes=Path(panel_path).read_bytes();panel=json.loads(panel_bytes);games=validate_panel(panel)
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=8) as pool:
            while time.monotonic()<deadline and not stop[0]:
                cycle=time.monotonic();now=time.time()
                active=[t for t,ko in tickers.items() if ko-604800<=now<ko-10800+300]
                jobs={pool.submit(get,'markets/'+t+'/orderbook',{'depth':10}):('book',t,None,None) for t in active}
                if cycle>=next_trade:
                    for t in active:
                        start=store.checkpoint(t,int(now)-3600)-120;end=int(now)-2
        ...
        # Close trade coverage through the last book receipt, not a fractional
        # timestamp truncated to the same second as that book. A query does not
        # establish that no more delayed prints will arrive in the future.
