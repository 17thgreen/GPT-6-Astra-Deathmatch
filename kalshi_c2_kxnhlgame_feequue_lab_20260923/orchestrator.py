"""C2 KXNHLGAME fee and queue honesty harness.
...
values. Panel markets are the scout-hunt objects. Lee-Ready is refused.
...
LAB_DIRECTORY = 'kalshi_c2_kxnhlgame_feequue_lab_20260923'
EXPERIMENT_ID = 'C2-KXNHLGAME-FEEQUEUE-HARNESS'
FEATURE_FAMILY = 'NHL-FQ'
SERIES = 'KXNHLGAME'
...
SCOUT_CITE = 'packets/scout_cashcow_hunt_2026-09-22/scout_hunt_KXNHLGAME.json'
KNOB = 'analysis_slice'
C2A0 = 'C2A0'
C2A1 = 'C2A1'
ARMS = (C2A0, C2A1)
...
BASE_COMMIT = 'ead2cb41d5d171d8972a27b98d144849b6f8c371'
FREEZE_SHA256 = 'a36ec35143a32c7cd24e9fdf2a33f645d356b93e34c131bb1b8a94e3f92e38f5'
SCOUT_SHA256 = '1ab794ad688ba31e0178e78294dcdbe50cf2799a71f40245e5a04a80e9dd5762'
PANEL_STUB_SHA256 = '60d183e7bdcf25adbc94eeeb3bb361b5232c19c0fe3e6a115f45ab3fcb100c79'
CONDUCTOR_STAMP_SHA256 = 'c5718b243a8d5491621bea3e382575d6b2e501c0cf507209c820f3d1fe118326'
PRE_ACCEPT_EMPTY_SHA256 = '88395e7bba510262bcac68565dbacc82ec52400335f5528135bd50fdaa06925b'
CONDUCTOR_ACCEPT_SHA256 = 'f0d5c7daa7c2eaa043572dddf52dc58757095bcd687feed978c502c6e9137422'
EXAMINER_HOLD_SHA256 = 'd2608d04765d988529fe988c577a66dfe2340be3cccd260382861520b6eb3209'
SOURCE_PINS_SHA256 = '2f1a9da6b5365c79300d8883847358beeb940f1f6ccf5a2e770a1ee136720ded'
FREEZE_BYTES = 5229
SCOUT_BYTES = 125008
PANEL_BYTES = 29517
STAMP_BYTES = 494
...
FREEZE_NAME = 'C2_KXNHLGAME_FEEQUEUE_HARNESS_FREEZE_2026-09-23.md'
SCOUT_NAME = 'scout_hunt_KXNHLGAME.json'
...
ACCEPT_NAME = 'CONDUCTOR_ACCEPT_C2_KXNHLGAME_FEEQUEUE_HARNESS_2026-09-23.json'
HOLD_NAME = 'EXAMINER_HOLD_C2_KXNHLGAME_FEEQUEUE_HARNESS_PRE_PR_2026-09-23.json'
SOURCE_PINS_NAME = 'SOURCE_PINS.json'
PACKET = ROOT / FREEZE_NAME
LAB_BUNDLE = ROOT / 'C2_KXNHLGAME_FEEQUEUE_HARNESS'
GOVERNANCE_BUNDLE = PARENT / 'packets' / 'C2_KXNHLGAME_FEEQUEUE_HARNESS'
...
PANEL_STUB = PARENT / 'lab' / 'astra-capture' / 'c2-kxnhlgame' / 'panel_stub.json'
...
def _load_hygiene():
    spec = importlib.util.spec_from_file_location('c2_kxnhlgame_hygiene', HYGIENE_PATH)
    if spec is None or spec.loader is None:
        raise OrchestratorError('hygiene import')
    module = importlib.util.module_from_spec(spec)
    sys.modules['c2_kxnhlgame_hygiene'] = module
...
def scout_index():
    """Authentic hunt: 66 markets, 33 events. Cached after the digest matches."""
    global _SCOUT_INDEX
...
def _assert_authentic_bytes():
    ...
    if accept.get('does_not_ungate') != list(DOES_NOT_UNGATE):
        raise UngateRefused()
    if accept.get('claims_s1_green') is not False:
        raise S1GreenClaimRefused()
    if accept.get('post_l2sf_main') != BASE_COMMIT:
        raise OrchestratorError('conductor accept')
    ...
    if accept.get('scout_markets') != SCOUT_MARKETS_N or accept.get('scout_events') != SCOUT_EVENTS_N:
        raise OrchestratorError('conductor accept')
    if accept.get('panel_subset_of_scout') is not True or accept.get('admitted_at') is not None:
        raise OrchestratorError('conductor accept')
    ...
    if hold.get('scout_hunt_sha256') != SCOUT_SHA256 or hold.get('panel_stub_sha256') != PANEL_STUB_SHA256:
        raise OrchestratorError('examiner hold')
    if sorted(hold.get('metrics_null') or []) != sorted(OUTPUT_KEYS):
        raise OrchestratorError('examiner hold')
    if hold.get('admitted_at') is not None or hold.get('panel_subset_of_scout') is not True:
        raise OrchestratorError('examiner hold')
...
def _validate_panel(payload, admitted):
    ...
    for event in events:
        ...
        occurrence = event.get('occurrence_datetime')
    ...
    for name in event_names:
        panel_n = sum(1 for market in markets if market.get('event_ticker') == name)
        scout_n = sum(1 for market in index['by_ticker'].values() if market.get('event_ticker') == name)
        if panel_n != 2 or scout_n != 2:
            raise InventedMarketRefused()
    return payload
...
def instrument_binding(panel=None):
    ...
    return {
        ...
        'probe_formula_id': nhl['formula_id'],
        'probe_series_resolution': nhl['series_resolution'],
...
def conduct_native(path=None):
    """C2A0 schema on synthetic native rows. Does not write the freeze scorecard."""
    rows = partition_native(load_synthetic_trades(path))
    return _report(C2A0, 'synthetic_schema_standin', {
        'partitions': rows,
...
def conduct_fresh(path=None):
    """C2A1 schema on synthetic book rows. The gap stays null."""
    rows = partition_fresh(load_synthetic_fresh(path))
    return _report(C2A1, 'synthetic_schema_standin', {
        'bins': rows,
        'bin_count': len(rows),
    })
