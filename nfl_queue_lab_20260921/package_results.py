"""Hash and archive the standalone experiment; no private repository sources included."""
...
def main():
    ...
    exclude={'NFL_Queue_Experiment_Kit.zip','DELIVERY_MANIFEST.json'}
    files=sorted(p for p in ROOT.rglob('*') if p.is_file() and '__pycache__' not in p.parts
                 and p.name not in exclude and not p.name.endswith('.pyc'))
    ...
    archive=ROOT/'NFL_Queue_Experiment_Kit.zip'
