# S4 KXNCAAFGAME settled-resolution join packet
...
`admitted_at` stays null. `results`, `pnl`, and `settled_join_n` stay
null. Scout nonempty N=18 is a pin and is not copied into
`settled_join_n`. This packet does not run `admit.py`.
