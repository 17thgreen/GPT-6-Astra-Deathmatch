"""Unit pins for the R2P3 KXNFLPASSYDS settled-resolution join harness.
...
class PinTests(unittest.TestCase):
    def test_binding_pins_the_seed_and_leaves_the_scorecard_null(self):
        ...
        self.assertEqual(binding['experiment_id'], 'R2P3-KXNFLPASSYDS-SETTLED-RESOLUTION-JOIN-HARNESS')
        self.assertEqual(binding['feature_family'], 'R2P3-RJ')
        self.assertEqual(frozen['feature_family'], 'R2P3-RJ')
        ...
        self.assertEqual(binding['market_tickers_n'], 0)
        self.assertIs(binding['panel_market_tickers_empty'], True)
        ...
        self.assertEqual(binding['yes_n'], 6)
        self.assertEqual(binding['no_n'], 14)
        ...
        self.assertIs(binding['parent_sep27_active_empty'], True)
        self.assertIs(binding['parent_reget_occurrence_present'], False)
        self.assertIs(binding['atl_gb_excluded'], True)
        self.assertIs(binding['list_429_backfilled'], False)
        self.assertIs(binding['close_window_backfilled'], False)
        ...
        self.assertEqual(binding['close_window_400'], '400_honest')
        self.assertEqual(binding['http_counts'], {'200': 5, '429': 3, '400': 1})
        self.assertEqual(binding['honest_gap_n'], 5)
        ...
        self.assertIs(binding['prop_ladder_reopen'], False)
        self.assertIs(binding['passyds_prop_reopen'], False)
        ...
        self.assertIs(pins['governance_r2p3_cites_present'], True)
    ...
    def test_packet_copies_match_and_scorecard_stays_null(self):
        ...
        self.assertEqual(orchestrator.sha256_file(orchestrator.PANEL_LADDER), orchestrator.PANEL_STUB_SHA256)
    ...
    def test_panel_stub_admitted_at_stays_null(self):
        ...
        panel = orchestrator.load_panel()
        self.assertEqual(panel['packet_id'], 'R2-P3-KXNFLPASSYDS-MEAS')
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            version['panel_version'] = '2026-09-22.r2-p3-prop-slate-v1'
    ...
    def test_reget_honesty_keeps_the_list_429_gaps(self):
        ...
        self.assertNotIn('parent_seeds_finalized_nonempty_N', reget)
        self.assertEqual(scout['http_counts'], {'200': 5, '429': 3, '400': 1})
        ...
        self.assertEqual(reget['list_http']['finalized'], '429_honest')
        self.assertEqual(reget['list_http']['events_closed'], '429_honest')
        self.assertEqual(reget['list_http']['events_settled'], '429_honest')
        self.assertEqual(reget['list_http']['closewin_min_max_close_ts'], '400_honest')
        ...
        self.assertEqual(sum(1 for row in passes if row['settled_list_row']), 20)
        self.assertEqual(sum(1 for row in passes if row['single_market_get']), 0)
        self.assertEqual(sum(1 for row in passes if row['event_embed']), 0)
        self.assertEqual(sum(1 for row in passes if row['result'] == 'yes'), 6)
        self.assertEqual(sum(1 for row in passes if row['result'] == 'no'), 14)
        self.assertEqual(sum(1 for row in active if row['parent_seed']), 4)
        self.assertTrue(all(row['j1'] == 'parent_occurrence_not_on_reget' for row in active))
        self.assertTrue(all(row['panel_ticker'] is None for row in active))
    ...
    def test_source_pins_and_accept(self):
        ...
        self.assertIs(pins['panel_market_tickers_empty'], True)
        ...
        self.assertEqual(pins['close_window_400'], '400_honest')
        self.assertIs(pins['fee_import_used'], False)
        self.assertIs(pins['rails_import_used'], False)
        self.assertIs(pins['prop_ladder_reopen'], False)
        self.assertIs(pins['passyds_prop_reopen'], False)
        ...
        self.assertIn('R2_P3_prop_ladder_reopen', accept['hard_refuse'])
        self.assertIn('PASSYDS_PROP_reopen', accept['hard_refuse'])
        self.assertIn('S4_RJ_reopen', accept['hard_refuse'])
    ...
    def test_invented_result_depth_fills_and_list_429_backfill_are_refused(self):
        ...
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.backfill_list_429(
                'markets?series_ticker=KXNFLPASSYDS&status=finalized',
                result='yes',
                occurrence_datetime='2026-09-21T03:20:00Z',
            )
        with self.assertRaises(orchestrator.InventedResultRefused):
            orchestrator.backfill_close_window('min_close_ts', result='yes')
        with self.assertRaises(orchestrator.InventedMarketRefused):
            orchestrator.follow_settled_cursor('present')
        with self.assertRaises(orchestrator.InventedResultRefused):
        ...
        with self.assertRaises(orchestrator.InventedSoTRefused):
            orchestrator.assign_occurrence(orchestrator.PARENT_SEED_TICKERS[0], '2026-09-27T20:00:00Z')
        with self.assertRaises(orchestrator.InventedResultRefused):
        ...
        invented['events'][0]['market_tickers'] = [orchestrator.PARENT_SEED_TICKERS[0]]
        with tempfile.TemporaryDirectory() as tmp:
